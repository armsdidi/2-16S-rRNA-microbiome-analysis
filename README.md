...
Os scripts Shell (`.sh`) contêm as etapas de download, controle de qualidade e pré-processamento inicial. Os scripts R (`.R`) contêm a inferência de ASVs, atribuição taxonômica, reconstrução filogenética, análises estatísticas, análises ecológicas e visualizações.
